 ...
 */
int shmem_create( const unsigned char name[ NAME_LENGTH ], 
					int pid, 
					int pages, 
					unsigned int flags )
{
 ...
 * \return 0 or greater - representing the ID of the memory region. 
 */
int shmem_create_direct( const unsigned char name[ NAME_LENGTH ], 
					int pid, 
					int pages, 
					unsigned int flags,
					uintptr_t location,
...
#warning This should not really be done, dude
	...
	// Change the information
		...
		sh->list = NULL;
		sh->lock = init_rwlock();
		strncpy( sh->name, name, NAME_LENGTH );
 ...
 */
int shmem_find( const unsigned char name[ NAME_LENGTH ], int *pid )
{
	int i = 0;
	struct shmem *sh;
	...
	{
		if ( strncmp( name, sh->name, NAME_LENGTH ) == 0 )
		{
			i = sh->id;
			*pid = sh->pid;
			rwlock_release( shmem_lock );
 ...
 */
int shmem_get( int num, unsigned char name[ NAME_LENGTH], 
						  int *pid, int *pages, 
						  unsigned int *flags )
{
	int i = 0;
	...
	while ( (sh = htable_get( shmem_table, i++ ) ) != NULL )
	{
		if ( i > num )
		{
			strncpy( name, sh->name, NAME_LENGTH );
			*pid = sh->pid;
