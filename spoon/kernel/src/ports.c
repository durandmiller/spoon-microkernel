...
int create_port( struct thread *tr, int port, int tid )
{
  ...
  proc->ipc[port].pulse_q = (struct ds_aqueue*)
		  							malloc( sizeof(struct ds_aqueue) );
  if ( proc->ipc[port].pulse_q == NULL ) return -1;

  proc->ipc[port].message_q = (struct ds_aqueue*)
		  							malloc( sizeof(struct ds_aqueue) );
  if ( proc->ipc[port].message_q == NULL ) 
  {
		free( proc->ipc[port].pulse_q );
		proc->ipc[port].pulse_q = NULL;
		return -1;
  }

  aqueue_init( proc->ipc[port].pulse_q, sizeof(struct pulse), PULSE_OBJECTS );
  aqueue_init( proc->ipc[port].message_q, 
...
int release_port( struct thread *tr, int port )
{
  ...
  if ( tr->tid == proc->ipc[port].tid ) target = tr;
	...
	aqueue_reset( proc->ipc[port].pulse_q );	// Clean the pulse queue
	aqueue_reset( proc->ipc[port].message_q );	// Clean the pulse queue
	free( proc->ipc[port].pulse_q );			// Free the memory
	free( proc->ipc[port].message_q );			// Free the memory
