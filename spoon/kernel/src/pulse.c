		     ...
		     unsigned int data6 )
{
	...
	// Let's get a new pulse to initialize.
		...
		aqueue_lock( proc->ipc[dest_port].pulse_q );
	
			p = (struct pulse*)aqueue_new( proc->ipc[dest_port].pulse_q );
			...
			aqueue_save( proc->ipc[dest_port].pulse_q );

		aqueue_unlock( proc->ipc[dest_port].pulse_q );
		     ...
		     unsigned int *data6 )
{
	...
	if ( (port >= 0) && (port < MAX_PORTS) )
	{
		...
		aqueue_lock( proc->ipc[port].pulse_q );

			p = (struct pulse*)aqueue_next( proc->ipc[port].pulse_q );
			if ( p == NULL )
			{
				aqueue_unlock( proc->ipc[port].pulse_q );
				return -1;
			}
		     ...
		     unsigned int *data6 )
{
	...
	for ( i = bottom; i < top; i++ )
	{
		...
		aqueue_lock( proc->ipc[port].pulse_q );
			...
			p = (struct pulse*)aqueue_get( proc->ipc[port].pulse_q );
			if ( p == NULL )
			{
				aqueue_unlock( proc->ipc[port].pulse_q );
				return -1;
...
int pulses_waiting( struct thread *tr )
{
	...
	for ( i = 0; i <  MAX_PORTS; i++ )
	{
	  ...
	  aqueue_lock( proc->ipc[ tr->ports[i] ].pulse_q );
	  count += aqueue_size( proc->ipc[ tr->ports[i] ].pulse_q );
	  aqueue_unlock( proc->ipc[ tr->ports[i] ].pulse_q );
    }
