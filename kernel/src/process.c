...
struct process *new_process( const unsigned char name[NAME_LENGTH] )
{
	int i;
...
int find_process( const unsigned char name[NAME_LENGTH] )
{
	int pid = -1;
	...
	rwlock_get_read_access( l_processLock );
		...
		while ( tmp != NULL )
		{
			if ( strncmp( name, tmp->name, NAME_LENGTH ) == 0 )
			{
				pid = tmp->pid;
				break;
			}
...
{
//	dmesg("%!(%i)Freeing %s(%i)\n", current_pid(), p->name, p->pid );
		
	rwlock_release( p->lock );
	rwlock_release( l_processLock );

//	if ( (current_pid() == 4) || (p->pid == 4) )
//	dmesg("%!(%i.%i)Freed %i\n", 
//					current_pid(), 
//					current_tid(), 
//					p->pid );
...
int		process_get_list( int max,  struct process_information* list )
{
	...
	{
		int rc = 0;
		strncpy( (unsigned char*)(list[done].name), 
				 (unsigned char*)(p->name), NAME_LENGTH );

		rc = get_environment( p, 
							(unsigned char*)"CMD_LINE", 
							list[done].cmdline, CMDLINE_LENGTH );
		assert( rc >= 0 );
