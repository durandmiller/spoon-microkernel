...
void init_irqs()
{
	...
	{
		handlers[i] = (struct irq_hook*)malloc( sizeof( struct irq_hook ) );
		handlers[i]->raised = 0;
		handlers[i]->index = 0;
		handlers[i]->last = 0;
		handlers[i]->count = 0;
...
int request_irq( struct thread *t, int irq )
{
	...
	hook = (struct irq_hook*)malloc( sizeof( struct irq_hook ) );
	...
	acquire_spinlock( &(irq_locks[irq]) );
		...
		hook->stubs = malloc( sizeof(struct irq_stub) * (hook->count+1) );
		if ( hook->stubs == NULL )
		{
	...
	if ( hook->stubs != NULL ) free( hook->stubs );
	free( hook );
...
int release_irq( struct thread *t, int irq )
{
	...
	acquire_spinlock( &(irq_locks[irq]) );
		...
		{
			hook->stubs = malloc( sizeof(struct irq_stub) * hook->count );
			if ( hook->stubs == NULL )
			{
				release_spinlock( &(irq_locks[irq]) );
				free( hook );
