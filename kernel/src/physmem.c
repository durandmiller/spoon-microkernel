...
volatile static uint32_t memory_bitmap[ MAXMAP ];


#define ISSET(block)	\
		(memory_bitmap[ ((block)/32) ] & (uint32_t)(1ul<<((block)%32))) != 0

#define SET(block)		\
			atomic_bitset( &(memory_bitmap[ ((block)/32) ]), (block)%32 );	\
			s_freePages -= 1

#define UNSET(block)		\
			atomic_bitclear( &(memory_bitmap[ ((block)/32) ]), (block)%32 );	\
			s_freePages += 1
